...
export function PreviewSurface({
    ...
    //
    // AND ON THE FALLBACK PATH BOTH SUBJECTS PAINT AN `<img>` AGAIN. When that
    // `<video>` errors — the keep-the-original sentinel, a decoder that has no
    // H.264, a dropped connection — GalleryImageLarge walks a two-rung ladder
    // (see its `loopFallback`): the BARE URL first, which for a sentinel is the
    // animating original at full size, and `still=true` only if that fails too.
    // Rung 2 is the peek's URL character for character, so the store hears the
    // same number twice there; at rung 1 the two subjects paint different
    // pictures of the same file, and the store keeps whichever reported — a
    // ratio either way, both of them the item's own.
    //
