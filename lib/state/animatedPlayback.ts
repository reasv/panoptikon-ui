...
//     when it last actually moved. It is what the HOVER ARMING machine asks
//     its question of (lib/state/hoverArming.ts, canArmHover), and it is the
//     whole standing cost of that feature on a page nobody hovers: a
//     comparison and two writes per event.
...
// RUNTIME: one listener lifetime, one dwell timer, one callback map, and the
// `fastScroll` sample the playback cap and the arming rule share.
...
/**
 * Carry out what a machine step decided: stop a cell that was playing, and
 * start a dwell for a cell that just took the slot. `null` is the common
 * answer and costs nothing.
 */
function applyHoverCommands(cmd: HoverCommands | null): void {
  if (!cmd) return
  if (cmd.cancelDwell) {
    clearTimeout(dwellTimer)
    dwellTimer = undefined
  }
  // Only a cell that was actually told to play needs telling to stop; a dwell
  // that never fired has nothing to take back.
  if (cmd.stop) hoverCallbacks.get(cmd.stop)?.(false)
  const starting = cmd.start
  if (!starting) return
...
export function armHoverPlay(
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    if (held && held !== onFire) {
      console.warn(
        "armHoverPlay: two armable pictures share one hover root; the first " +
        "will never be told to stop",
        root
      )
    }
  }
  hoverCallbacks.set(root, onFire)
  applyHoverCommands(hoverEnter(
    hoverArming,
    root,
  ...
  return () => {
    applyHoverCommands(hoverLeave(hoverArming, root))
    // The callback goes with the arm it belonged to. Left behind, a WeakMap
    // entry for a root that is still on the page would let a LATER arm under
    // that root — the director's own pending-candidate retry, or a second
    // picture mounting into it — reach a cell that has already released.
    hoverCallbacks.delete(root)
  }
}
...
function maybeTeardown(): void {
  ...
  if (hoverArming.armed) {
    applyHoverCommands(hoverLeave(hoverArming, hoverArming.armed))
  }
  clearTimeout(dwellTimer)
  dwellTimer = undefined
  initHoverArming(hoverArming)
...
function onScroll(event: Event): void {
  ...
  if (hoverArming.armed) {
    applyHoverCommands(hoverScroll(hoverArming, false))
  }
  ...
  applyHoverCommands(hoverScroll(hoverArming, true))
  reconcile()
}
...
/**
 * Test seam: the numbers the PLAYBACK policy is written in. The hover rule's
 * two constants are exported from lib/state/hoverArming.ts, next to the rule
 * they belong to, rather than mirrored here.
 */
